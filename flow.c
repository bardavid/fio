...
int flow_threshold_exceeded(struct thread_data *td)
{
	...
	flow_counter_ratio = (double)td->flow_counter / atomic_load_relaxed(&flow->flow_counter);
	flow_weight_ratio = (double)td->o.flow / atomic_load_relaxed(&flow->total_weight);
	...
	* ratio between them is proportional to other jobs in the same flow_id */
	...
	atomic_inc(&flow->flow_counter, 1);
	++td->flow_counter;
...
static void flow_put(struct fio_flow *flow, unsigned long long flow_counter, unsigned int weight)
{
	if (!flow_lock)
		return;
...
void flow_init_job(struct thread_data *td)
{
	if (td->o.flow) {
		...
		td->flow_counter = 0;
		atomic_inc(&td->flow->total_weight, td->o.flow);
	}
}
